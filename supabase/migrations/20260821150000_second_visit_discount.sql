-- Second-visit discount on affiliation + serial flag

ALTER TABLE public.care_affiliations
  ADD COLUMN IF NOT EXISTS second_visit_discount_type TEXT
    CHECK (second_visit_discount_type IS NULL OR second_visit_discount_type IN ('percent', 'fixed')),
  ADD COLUMN IF NOT EXISTS second_visit_discount_value NUMERIC(12, 2);

ALTER TABLE public.care_serials
  ADD COLUMN IF NOT EXISTS is_second_visit BOOLEAN NOT NULL DEFAULT false,
  ADD COLUMN IF NOT EXISTS fee_original NUMERIC(12, 2);
...
BEGIN
  ...
  IF require_approval THEN
    INSERT INTO public.care_serials (
      ...
      source, status, fee_amount, fee_original, is_second_visit, payment_status
    )
    VALUES (
      sess.id, NULL, _patient_id, NULLIF(_guest_name, ''), NULLIF(_guest_phone, ''), g_age,
      NULLIF(_guest_address, ''), src, 'pending_approval', fee, fee_base, second_visit, 'pending'
    )
    ...
    PERFORM public.care_write_audit(sess.org_id, 'serial.request', 'care_serials', ticket.id,
      jsonb_build_object(
        ...
        'is_second_visit', second_visit, 'status', 'pending_approval'
      ));
  ...
  VALUES (
    ...
    NULLIF(_guest_address, ''), src, 'booked', fee, fee_base, second_visit, 'pending'
  )
  RETURNING * INTO ticket;
