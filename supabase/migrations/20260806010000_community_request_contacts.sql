-- Community contact log (not stored in feed notes) + 3-month donor cooldown
-- Run on Supabase SQL editor or via migrate.
...
-- One open "initiated" row per request+phone+channel (allow re-contact after donate)
...
-- 3 months cooldown on confirmed donation
...
-- Restore availability when cooldown ends (callable from app / cron)
...
-- Mark community donor + matched profile unavailable for 3 months
...
BEGIN
  ...
  -- Permission: request owner, org member, or admin
  ...
  IF v_profile_id IS NOT NULL THEN
    ...
    -- trigger sets profile unavailable
  END IF;
