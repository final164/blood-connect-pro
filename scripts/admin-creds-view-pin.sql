...
BEGIN
  ...
  -- Fallback: auth user metadata (if PIN was stored on signup)
  ...
  IF v_pin IS NULL OR v_pin = '' THEN
    -- Last resort: profile phone only (pin still missing)
...
-- Also tighten SELECT policy (same checks)
...
-- Ensure credentials table has rows for users who have profile phone (PIN unknown stays out)
-- No fake PIN insert — only fix read path.
