...
import { getDhakaUpazilaNeighborSlugPairs } from "../src/data/upazila-neighbors";

const districtValues = DISTRICT_NEIGHBOR_PAIRS.map(([a, b]) => `  ('${a}', '${b}')`).join(",\n");
const dhakaValues = getDhakaUpazilaNeighborSlugPairs()
  .map(([a, b]) => `  ('dhaka', '${a}', '${b}')`)
  .join(",\n");

const sql = `-- Proximity ranking: district/upazila neighbor graphs + hop precompute + fetch_ranked_feed rewrite
-- Safe to re-run.
...
writeFileSync("scripts/feed-proximity-ranking.sql", sql);
writeFileSync("supabase/migrations/20260730190000_feed_proximity_ranking.sql", sql);
