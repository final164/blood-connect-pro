...
import { mapSerialInvoiceToViewModel } from "@/lib/care-invoice-view-model";
...
export function CareSerialInvoiceCard({
  ...
  const printRootId = `care-serial-invoice-${serialId}`;
  ...
  async function downloadPdf() {
    ...
    setDownloading(true);
    try {
      const serialPart = invoice.serial_no ?? invoice.online_serial_no ?? "pending";
  ...
  const vm = mapSerialInvoiceToViewModel(invoice, template, lang);
